...
function renderOverrunDetail(key) {
  ...
  // --- Budget side ---
  let budgetHtml;
  if (d.budgetLines.length) {
    const linesTotal = d.budgetLines.reduce((sum, l) => sum + l.amount, 0);
    budgetHtml = `
      <table class="drill-table">
        ...
        <tfoot>
          <tr>
            <td>Budgeted lines${Math.abs(linesTotal - d.budget) > 0.01 ? ` <em>(sheet total ${fmt(d.budget)})</em>` : ""}</td>
            <td class="num">${fmt(linesTotal)}</td>
          </tr>
        </tfoot>
      </table>`;
  } else {
    budgetHtml = `
      <p class="drill-empty">The sheet holds one figure for this subcategory — <strong>${fmt(d.budget)}</strong> — with no line-level breakdown behind it. Set <code>detailRange</code> in <code>SUBCATEGORY_BUDGET_SOURCE</code> to itemise it here.</p>`;
  }

  // --- Actual side ---
  let actualHtml;
  if (d.actualLines.length) {
    actualHtml = `
      <table class="drill-table">
        <tbody>
          ${d.actualLines.map(r => {
            const share = d.actual > 0 ? (r.total / d.actual) * 100 : 0;
            const qtyBit = r.qty ? `${esc(r.qty)}${r.uom ? " " + esc(r.uom) : ""} × ${esc(r.unitCost)}` : "";
            const meta = [qtyBit, esc(r.description)].filter(Boolean).join(" · ");
            return `
              <tr>
                <td>
                  <span class="drill-item">${esc(r.item) || "—"}</span>
                  ${meta ? `<span class="drill-meta">${meta}</span>` : ""}
                </td>
                <td class="drill-date">${r.date.toLocaleDateString("en-US", { month: "short", day: "numeric" })}</td>
                <td class="num">${fmt(r.total)}<span class="drill-share">${share.toFixed(0)}%</span></td>
              </tr>`;
          }).join("")}
        </tbody>
        <tfoot>
          <tr>
            <td colspan="2">${d.actualLines.length} ${d.actualLines.length === 1 ? "entry" : "entries"} this month</td>
            <td class="num">${fmt(d.actual)}</td>
          </tr>
        </tfoot>
      </table>`;
  } else {
    actualHtml = `<p class="drill-empty">No individual entries found for this subcategory this month, yet the sheet shows spend against it — the actual figure may be coming from a row whose sub category name doesn't match the budget exactly.</p>`;
  }

  return `
    <div class="drill-section">
      <div class="drill-head">Budget</div>
      ${budgetHtml}
    </div>
    <div class="drill-section">
      <div class="drill-head">What was actually spent</div>
      ${actualHtml}
    </div>
    <div class="drill-verdict">
      Over by <strong>${fmt(d.over)}</strong>
    </div>`;
}
